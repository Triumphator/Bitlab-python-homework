...
def addMovie(year, movie_title, budget, director, box_office):
    global mydb
    mycursor = mydb.cursor()
    sql = "INSERT INTO movies (year, movie_title, budget, director, box_office) VALUES (%s, %s, %s, %s, %s)"
...
def onlineShop(movie_title):
    global mydb
    mycursor = mydb.cursor()
    sql = "SELECT movies.Movie_title, shops.ShopName FROM movies INNER JOIN shops ON movies.Movie_title = shops.Movie_title; " \
          "WHERE Movie_title = " + str(movie_title)
    mycursor.execute(sql)
    mydb.commit()
...
while True:
    ...
    if 0 <= choice <= 5:
        if choice == 1:
            ...
            confirmation = input("Are you sure?\n Press Y to confirm and N to reject\n")
            if confirmation == "y":
            ...
            movies = getMovies()
            print("Enter movie title to check"
                  "if it is available for purchase")
            movie_title = input()
            onlineShop(movie_title)
